...
class XOpenSwitchPlugin(snapcraft.BasePlugin):
    ...
    def schema(cls):
        ...
        schema['properties']['skip-native-build'] = {
            'type': 'boolean',
            'default': 0,
        }
    ...
    def _relocate_debug_info(self,debug_info):
        if not os.path.exists(debug_info):
            return
        logger.info('Relocating debug information.')
    ...
    def _set_interpreter(self, new_interpreter):
        logger.info('Change ELF interpreter setting to ' +  new_interpreter)
    ...
    def build(self):
        ...
        """ Since we are pre-building, builddir simply links to sourcedir
            In other words, the objects generated by the native OpenSwitch
            build are located in the source directory, not the build.

            The primary reason for this choice is that the source directory is
            fairly large and has fairly complex links, etc.  We're trying to
            treat the OpenSwitch native build as a stand-alone and almost
            independent operation.
        """
        if os.path.exists(self.build_basedir):
            if not os.path.islink(self.build_basedir):
                logger.info('Build overrides link ' + self.build_basedir + ' ==> ' + self.sourcedir)
                shutil.rmtree(self.build_basedir)
                os.symlink(self.sourcedir, self.build_basedir)

        """ If openswitch is not yet configured, configure it.
            Force a native build if we had to configure.
        """
        if not os.path.exists(os.path.join(self.sourcedir, '.platform')):
            logger.info('OpenSwitch not configured; configuring ' + self.options.platform + '; forcing a native build.')
            self.run(['make', 'configure', self.options.platform])
            self.options.skip_native_build = 0
        ...
        """
        if not self.options.skip_native_build:
            self.run(command)
        else:
            logger.info('Build skipping OpenSwitch native build...')
